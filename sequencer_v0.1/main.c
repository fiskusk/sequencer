...
ISR(TIMER1_OVF_vect)
{
	switch(current_state){
		...
		case fault:
		   ...
		   if (once == 1)
		   {
			   uart_puts("nastal fault - fault postupne vse vypnu a drzím delsi dobu, tedy tohle vse vcetne vypinani vseho delam znovu...\n");
			   pom = "Switch OFF Ucc     ";
			   _delay_ms(TSeq);
			   pom = "Switch OFF bias    ";
			   _delay_ms(TSeq);
		   ...
		   {
			   uart_puts("jak probehne x opakovani faultu (v prvnim startu prednastaveny jenom na jedno projetí), pak zkontroluju jestli je uz vse OK SKOKEM na after fault\n");
