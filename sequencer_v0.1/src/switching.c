...
void switching_relay2(state_t state)
{
    ...
    pom = "sw rel2 ON/OFF   ";
    (state == ENABLE) ? (SWITCHING_RELAY2_PORT |= 1<<SWITCHING_RELAY2_PIN_NUM) : (SWITCHING_RELAY2_PORT &= ~(1<<SWITCHING_RELAY2_PIN_NUM));
}
...
void switching_bias(state_t state)
{
    ...
    pom = "sw bias ON/OFF   ";
    (state == ENABLE) ? (SWITCHING_BIAS_PORT |= 1<<SWITCHING_BIAS_PIN_NUM) : (SWITCHING_BIAS_PORT &= ~(1<<SWITCHING_BIAS_PIN_NUM));
}
...
void switching_ucc(state_t state)
{
    ...
    (state == ENABLE) ? (SWITCHING_UCC_DDR |= 1<<SWITCHING_UCC_PIN_NUM) : (SWITCHING_UCC_PORT &= ~(1<<SWITCHING_UCC_PIN_NUM));
}
...
void switching_fan(state_t state)
{
    ...
    pom = "sw fan ON/OFF   ";
    (state == ENABLE) ? (SWITCHING_FAN_PORT |= 1<<SWITCHING_FAN_PIN_NUM) : (SWITCHING_FAN_PORT &= ~(1<<SWITCHING_FAN_PIN_NUM));
}
